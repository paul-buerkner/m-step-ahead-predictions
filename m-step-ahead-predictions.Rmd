...
Lastly, we are going to illustrate 1-SAP when only leaving out a block of $B$ future observations while keeping more distant future observations (abbreviated as block-1-SAP). This will decrease Pareto-k-estimates on average, but it only a valid approximation of exact 1-SAP if distant futrue values contain little information about the currently left-out observation. 

### Exact 1-step-ahead predictions

The related code changes little with the exception that we now define a block size of left-out observations, which we set to $B = 10$ for the present case study.
