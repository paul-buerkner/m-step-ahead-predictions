...
$$
Having obtained $S$ draws $(\theta_{<i}^{(1)}, \ldots, \theta_{<i}^{(S)})$ from the posterior distribution $p(\theta\,|\,\yy{<i})$, we can estimate $p(\yy{i<M} | \yy{<i})$ by means of
...
are discussed in a separate vignette [_Leave-one-out cross-validation for non-factorizable models_](http://mc-stan.org/loo/articles/loo2-non-factorizable.html).
...
Autoregressive (AR) models are some of the most commonly used time-series
models. An AR-model of order $p$ can be defined as
...
To illustrate the application of $M$-SAP, we will use the annual measurements of the level (in feet) of Lake Huron 1875–1972 as a case study, which is shipped natively with **R**.
...
The most basic version of $M$-SAP is 1-SAP in which we predict only one step ahead. In this case, $\yy{i<M}$ simplifies to $y_{i}$ and the above described algorithm becomes considerably simpler.
...
The approximate 1-SAP elpd is remarkably similar to the exact 1-SAP elpd compute above, which indicates our algorithm to compute approximate 1-SAP worked well for the present data and model.
