...
log_ratios <- function(ll, ids = NULL) {
...
for (i in N:(L + 1)) {
  logratio[, i] <- log_ratios(loglik, i:N)
...
for (i in N:(L + 1)) {
  ...
  logratio[, i] <- log_ratios(loglik, i:i_refit)
  psis_part <- psis(logratio[, i:i_refit], r_eff = NA)
  ks <- pareto_k_values(psis_part)
  max_k <- c(max_k, max(ks))
  if (any(ks > k_thres)) {
...
```{r, cache=FALSE}
plot_max_k <- function(max_k, ids) {
  dat_max_k <- data.frame(max_k = rev(max_k), ids = ids)
  ggplot(dat_max_k, aes(x = ids, y = max_k)) + 
    geom_point(aes(color = max_k > 0.7, shape = max_k > 0.7), show.legend = FALSE) + 
    geom_hline(yintercept = 0.7, linetype = 2, color = "red2") + 
    scale_color_manual(values = c("black", "red3")) + 
    scale_shape_manual(values = c(19, 8)) +
    labs(x = "Data point", y = "max(Pareto k)") + 
    ylim(-0.5, 1.5)
}
plot_max_k(max_k, ids = (L+1):N)
```
...
```{r, echo=FALSE}
max_diff <- with(dat_elpd, max(abs(approx_elpd - exact_elpd)))
mean_diff <- with(dat_elpd, mean(abs(approx_elpd - exact_elpd)))
```
...
```{r, cache = FALSE}
plot_max_k(max_k, ids = (L+1):(N - M + 1))
```
...
for (i in N:(L + 1)) {
  ...
  logratio[, i] <- log_ratios(loglik, i:to)
  psis_part <- psis(logratio[, i:i_refit], r_eff = NA)
  ks <- pareto_k_values(psis_part)
  max_k <- c(max_k, max(ks))
...
have a time-series of 98 observations and are predicting for 
$98-L = $ `r 98 - L` of them.
