...
Depending on the particular time-series model and data, the Pareto-k-values will exceed $0.7$ rather quickly (i.e. after only few observations) and thus many refits may be required even if applying the above described algorithm. In this case, an option is to exclude only a number of $B$ future values which directly follow the observations to be predicted while keeping all more distant future values $B+1$ to $N$. This will usually result in lower Pareto-k-values and thus less refitting. The block-m-SAP resembles the basic m-SAP closely only if values in the distant future (that is farer away than $B$ steps) contain little information about the current observations being predicted. Whether this assumption is justified depends on the data and applied time-series model.
...
For the purpose of later comparison, we perform approximate leave-one-out cross-validation (LOO) as baseline whos accurarcy for 1-SAP we seek to improve.
...
if (L == 0) {
  # no prior observations for the first observation
  fit_i <- update(fit, sample_prior = "only", recompile = FALSE, chains = 1)
  loglik[, 1] <- log_lik(fit_i, newdata = df[1, ])[, 1]
}
```
...
The `r outlier0.5`th observation(s) show a distance between exact and approximate elpd of more than $0.5$. The `r outlier0.25`th observation(s) show a distance between exact and approximate elpd of more than $0.25$.
...
if (L == 0) {
  # no past observations for the first observation
  fit_i <- update(fit, sample_prior = "only", recompile = FALSE, chains = 1)
  ll <- log_lik(fit_i, newdata = df[1:m, ])
  loglikm[, 1] <- rowSums(ll[, 1:m])
}
```
