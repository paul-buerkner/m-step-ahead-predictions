...
```{r, cache = FALSE}
psis_full <- psis(logratio[, (L + 1):N], r_eff = NULL)
plot(psis_full)
...
for (i in N:(L + 1)) {
  ...
  logratio <- log_ratios(loglik, i:i_refit)
...
```{r, cache=FALSE}
plot_ks <- function(ks, ids) {
  dat_ks <- data.frame(ks = rev(ks), ids = ids)
  ggplot(dat_ks, aes(x = ids, y = ks)) + 
    geom_point(aes(color = ks > 0.7, shape = ks > 0.7), show.legend = FALSE) + 
...
for (i in N:(L + 1)) {
  ...
  logratio <- log_ratios(loglik, i:to)
  psis_part <- suppressWarnings(psis(logratio))
  k <- pareto_k_values(psis_part)
  ks <- c(ks, k)
  if (k > k_thres) {
