...
author: Paul Buerkner, Aki Vehtari, Jonah Gabry
output:
...
If there were no time dependence in the data we could use methods like
...
many times to different subsets of the data, which is infeasible for most
...
predictive performance starting with the $y_{11}$th observation so that we
...
$\prod_{j = i}^{i + M -1} p(y_j \,|\, \theta_{<i})$. Non-factorizable models
are discussed in a separate vignette 
[_Leave-one-out cross-validation for non-factorizable models_](http://mc-stan.org/loo/articles/loo2-non-factorizable.html).
...
algorithm. Starting at the end of the time-series (i.e. $i = N - M + 1$), we
approximate each $p(\yy{i<M} \,|\, \yy{<i})$ using Pareto smoothed importance
sampling:
...
We then decrease $i$ by $1$ and repeat the process. At some observation $i$, the
...
distribution first cross $0.7$ (Vehtari et al, 2017ab). Only then do we refit
the model using only observations before $i^\star_1$ and then restart the
process. In some cases we may only need to refit once and in other cases we will
find a value $i^\star_2$ that requires a second refitting, maybe an $i^\star_3$
that requires a third refitting, and so on. We repeat the refitting as few times
as is required (only if $k > 0.7$) until we arrive at $i = L+1$.  Recall that
$L$ is the minimum number of observations we have deemed acceptable for making
...
We can specify an AR(2) model for these data using the **brms** package as
follows:
...
$L = 15$ observations to make predictions.
...
loglik <- log_lik(fit)
logratio <- matrix(nrow = nsamples(fit), ncol = N)
for (i in N:(L + 1)) {
  logratio[, i] <- -rowSums(loglik[, i:N, drop = FALSE])
  psis_part <- psis(logratio[, i:N], r_eff = NA)
  w_i <- weights(psis_part, log = FALSE)[, 1]
...
corresponds to the point $i^\star_1$ discussed in the section
**Approximate $M$-SAP using importance-sampling** above.

```{r, cache = FALSE}
psis_full <- psis(logratio[, (L + 1):N], r_eff = NA)
plot(psis_full)
abline(v = which.max(pareto_k_ids(psis_full, threshold = 0.7)))
```
...
```{r, cache = FALSE}
approx_elpd_1sap_no_refit <- c(ELPD = sum(approx_elpds_1sap_no_refit, na.rm = TRUE))
rbind(
...
Note that we can compute exact 1-SAP at the refitting points, which comes with no
additional computational costs since we had to refit the model anyway.
...
for (i in N:(L + 1)) {
  ...
  logratio[, i] <- - rowSums(loglik[, i:i_refit, drop = FALSE])
  psis_part <- psis(logratio[, i:i_refit], r_eff = NA)
  if (any(pareto_k_values(psis_part) > k_thres)) {
...
of 0.7 and that we only needed to refit the model `r length(refits)` times at
observations `r refits`.

```{r, cache = FALSE}
plot(psis(logratio[, (L+1):N], r_eff = NA))
```
...
value deviates far from its exact counterpart providing further evidence for the
good quality of our approximation.
...
xy_lim <- range(dat_elpd, na.rm=TRUE)

ggplot(dat_elpd, aes(x = approx_elpd, y = exact_elpd)) +
  geom_abline(color = "gray30") +
  geom_point(size = 2) +
  labs(x = "Approximate ELPDs", y = "Exact ELPDs") +
  lims(x = xy_lim, y = xy_lim)
```

```{r, include=FALSE}
max_diff <- max(abs(dat_elpd$approx_elpd - dat_elpd$exact_elpd))
mean_diff <- mean(abs(dat_elpd$approx_elpd - dat_elpd$exact_elpd))
```
...
To illustrate the application of $M$-SAP if $m > 1$, we compute exact and
approximate 4-SAP for the same AR(2) model we fit to the Lake Huron data.
...
loglikm <- loglik <- logratio <- matrix(nrow = nsamples(fit), ncol = N)
...
```{r, cache = FALSE}
is_na <- apply(logratio, 2, anyNA)
plot(psis(logratio[, !is_na], r_eff = NA))
```
...
xy_lim <- range(dat_elpd_4sap, na.rm=TRUE)
...
Comparing the exact block-1-SAP to the exact 1-SAP reveals a substantial difference. 
...
This does, however, not necessarily imply that observations depend on distant future values. Rather, the reason could be that the model parameters were estimated more precisely in the block-1-SAP as more data are available in particular for computing 1-SAP of observations early in the time-series. If so, we would expect the differences between pointwise ELPDs of block-1-SAP and 1-SAP to be larger for earlier observations. And indeed, this is what we see when plotting the differences.
...
ggplot(dat_elpd, aes(x = i, y = elpd_diff)) +
  geom_point(size = 2) +
  xlab("Data Point") +
  ylab("Difference of exact elpds")
```
...
loglik <- logratio <- matrix(nrow = nsamples(fit), ncol = N)
...
for (i in N:(L + 1)) {
  ...
  logratio[, i] <- - rowSums(loglik[, i:to, drop = FALSE])
  psis_part <- psis(logratio[, i:i_refit], r_eff = NA)
...
* Code &copy; 2018, Paul Buerkner, Aki Vehtari, Jonah Gabry licensed under BSD-3.
* Text &copy; 2018, Paul Buerkner, Aki Vehtari, Jonah Gabry licensed under CC-BY-NC 4.0.
