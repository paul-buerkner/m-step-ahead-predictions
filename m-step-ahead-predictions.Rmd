...
log-density values of interest are now the sums of the log predictive 
densities of four consequtive observations.
...
```{r, cache = FALSE}
plot(psis(logratio[, (L+1):(N - M + 1)], r_eff = NA))
```

Similar to the 1-SAP case, the approximate ELPD computed for the 4-SAP case 
is very close to its exact counterpart.
...
may exceed $0.7$ rather quickly (i.e., after only few observations) and so
...
retaining all of the more distant values $\yy{>B} = (y_{B+1}, \ldots, y_N)$. 
...
The differences between the code for LFO-CV for 1-SAP and the codfe for
...
Comparing the exact block-1-SAP to the exact 1-SAP reveals a substantial
difference:
...
This does not necessarily imply that observations depend on distant future values,
but could instead be due to the fact that the model parameters were estimated 
more precisely in the case of block-1-SAP since more data are available, 
in particular for predicting observations early in the time series. 
If this is the case we would expect the differences between the pointwise 
ELPDs for the two cases to be larger for earlier observations. Indeed, this is
what we see when plotting the differences:
...
We needed only a single refit at observation `r refits`, which is remarkable
given that we have a time-series of 98 observations and are predicting
for $98-L = $`r 98 - L` of them.
